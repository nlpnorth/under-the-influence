...
def tokenize_file(input_path, output_path, tokenizer, max_seq_len,
    ...
    def tokenize_batch():
        ...
        for tokenized_line in batch_encoding["input_ids"]:
            candidate = curr_example + tokenized_line
            if sep_token_id is not None: candidate = candidate + [sep_token_id]
            if len(candidate) >= max_seq_len or curr_n_segments + 1 >= max_segments:
                # Flush current example before appending this sentence.
                if len(curr_example) > (1 if cls_token_id is not None else 0):
